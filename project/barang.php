...
@include "controlers/hapusBarang.php";
session_start();
...
$sizeDataPage = 5;
$totalData = count(query("SELECT barang.id_barang, jenis_barang.jenis_barang, barang.model, barang.harga, barang.ukuran, barang.stok FROM barang INNER JOIN jenis_barang ON barang.id_jbarang = jenis_barang.id_jbarang WHERE id_barang LIKE '%$keyword%' OR jenis_barang LIKE '%$keyword%' OR model LIKE '%$keyword%' OR harga LIKE '%$keyword%' OR ukuran LIKE '%$keyword%'"));
...
$dataPage = query("SELECT barang.id_barang, jenis_barang.jenis_barang, barang.model, barang.harga, barang.ukuran, barang.stok FROM barang INNER JOIN jenis_barang ON barang.id_jbarang = jenis_barang.id_jbarang WHERE id_barang LIKE '%$keyword%' OR jenis_barang LIKE '%$keyword%' OR model LIKE '%$keyword%' OR harga LIKE '%$keyword%' OR ukuran LIKE '%$keyword%' ORDER BY id_barang ASC LIMIT $firstData,$sizeDataPage");
          ...
          <div class="card shadow mb-4" style="margin-top: -20px;">
            ...
            <form action="" method="post">
              <div class="d-flex justify-content-between ">
                <div class="p-2 mx-2 mt-2"><a href="tambahBarang.php" class="btn btn-sm btn-primary"><i class="fa-solid fa-plus"></i><span class="d-none d-sm-inline mx-1">Tambah Barang</span></a></div>
                  ...
                  <tbody>
                    ...
                    <?php foreach ($dataPage as $row) : ?>
                      <tr>
                        <td><?= $i++ + $firstData; ?></td>
                        <td><?= $row["id_barang"]; ?></td>
                        <td class="col-md-2"><?= $row["jenis_barang"]; ?></td>
                        <td><?= $row["model"]; ?></td>
                        <td class="f-num"><?= $row["harga"]; ?></td>
                        <td><?= $row["ukuran"]; ?></td>
                        <td class="f-num"><?= $row["stok"]; ?></td>
                        <td class="col-md-2">
                          <div class="d-grid gap-2 d-md-block">
                            <a href="ubahBarang.php?id=<?= $row["id_barang"];  ?>" class="btn btn-success btn-sm" type="button"><i class="fa-solid fa-pen-to-square"></i></a>
                            <a href="barang.php?id=<?= $row["id_barang"];  ?>" class="btn btn-danger btn-sm" type="button"><i class="fa-solid fa-trash"></i></i></a>
                          </div>
                        </td>
                      </tr>
                    <?php endforeach ?>
    ...
    <footer class="py-4 mt-auto">
      <div class="container-fluid px-4">
        <div class="d-flex  justify-content-center small">
          <div class="text-muted">Copyright &copy; Kelompok 1 2023</div>
        </div>
      </div>
    </footer>
  </div>
